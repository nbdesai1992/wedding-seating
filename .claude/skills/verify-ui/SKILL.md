...
1. **Start the dev server** (if not already running):
   ```bash
   cd frontend && npx serve public -l 3000 &
   ```
...
const page = await browser.getPage("wedding-seating");
...
- Keep named page `"wedding-seating"` consistent across all dev-browser calls so state persists.
...
```bash
dev-browser --headless <<'SCRIPT'
const page = await browser.getPage("wedding-seating-mobile");
